...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        Many locations are using "City" to store:
         - "City, State"
         - "City, State, Zip" 

        https://developers.google.com/maps/documentation/geocoding/#Limits
        "Use of the Google Geocoding API is subject to a query limit of 2,500 
        geolocation requests per day. (User of Google Maps API for Business 
        may perform up to 100,000 requests per day.)

        A one second interval did not seem to be sufficient.

        """
        for loc in orm['companies.CompanyAddress'].objects.all():
            # if it ends in a zipcode, pop and move it
            zip_code_match = re.search(r'.*(\d{5}(\-\d{4})?)$', loc.city)
            if zip_code_match:
                loc.postal_code = zip_code_match.group(1)
                loc.city = loc.city.rstrip(loc.postal_code).strip()
            # if it ends in a state (only "TN"), pop and move it
            loc.city = loc.city.rstrip(' TN')
            loc.city = loc.city.rstrip(',')
            # this will invoke the geocode_address() method on the model
            loc.save()
            # stall for 2 second between geocode requests
            time.sleep(2)
